<template>
    ...
    <div class="w-full flex justify-between items-center px-3 py-3 border-b border-b-gray-300">
        ...
        <div class="relative" id="userDropdown">
            <div
                class="cursor-pointer size-[55px] bg-gray-700 font-medium text-white rounded-full inline-flex justify-center items-center"
                @click="openUserDropdown()">
                {{ shortName(profileData?.name) }}
            </div>
            <ul class="absolute top-auto end-0 min-w-[210px] p-2 bg-white drop-shadow-xl border mt-2 rounded-lg z-50"
                v-if="isUserDropdown" @click.stop>
                <li>
                    <button type="button"
                            class="cursor-pointer decoration-0 block w-full px-5 min-h-[48px] max-h-[48px] font-medium text-start bg-transparent duration-500 hover:bg-gray-300 rounded-lg"
                            @click="closeUserDropdown();openEditProfileModal()">
                        Edit Profile
                    </button>
                </li>
                <li>
                    <button type="button"
                            class="cursor-pointer decoration-0 block w-full px-5 min-h-[48px] max-h-[48px] font-medium text-start bg-transparent duration-500 hover:bg-gray-300 rounded-lg"
                            @click="closeUserDropdown();openChangePasswordModal()">
                        Change Password
                    </button>
                </li>
                <li>
                    <button type="button"
                            class="cursor-pointer decoration-0 block w-full px-5 min-h-[48px] max-h-[48px] font-medium text-start bg-transparent duration-500 hover:bg-gray-300 rounded-lg"
                            v-if="!logoutLoading" @click="logout()">
                        Logout
                    </button>
                    <button type="button"
                            class="cursor-pointer decoration-0 flex justify-center items-center w-full px-5 min-h-[48px] max-h-[48px] font-medium text-start bg-transparent duration-500 hover:bg-gray-300 rounded-lg"
                            v-if="logoutLoading">
                        <span
                            class="inline-block size-[20px] rounded-full border-2 border-transparent border-t-2 border-t-black animate-spin"></span>
                    </button>
                </li>
            </ul>
        </div>
    ...
    <div class="w-full flex justify-start items-start h-[calc(100vh-178px)] overflow-hidden">
        ...
        <div class="min-w-[350px] h-[calc(100vh-178px)] border-e border-e-gray-300">
            ...
            <div class="p-3 w-full">
                <input type="text" name="keyword" v-model="keyword" @input="searchData()" placeholder="Search Here ..."
                       class="w-full px-5 min-h-[50px] outline-0 block border-0 bg-gray-200 rounded-lg duration-500 focus-within:border-blue-700">
            </div>
            ...
            <div class="p-3 w-full scrollbar h-[calc(100vh-250px)]">
                ...
                <template v-for="each in showList" v-if="userData.length > 0">
                    <button type="button"
                            class="w-full flex justify-start items-center bg-transparent duration-500 hover:bg-gray-200 p-3 rounded-lg"
                            @click="selectUser(each)">
          <span
              class="min-w-[50px] relative min-h-[50px] size-[50px] inline-flex justify-center rounded-full items-center bg-gray-600 text-white">
                ...
                <div class="w-full h-[calc(100vh-175px)]">
                    ...
                    <div class="w-full flex justify-between items-center p-3">
                        ...
                        <div class="relative" id="otherUserDropdown">
                            <button type="button"
                                    class="size-[50px] min-w-[50px] min-h-[50px] rounded-full inline-flex justify-center items-center bg-transparent duration-500 hover:bg-gray-300"
                                    @click="openOtherUserDropdown()">
                            ...
                            <ul class="absolute top-auto end-0 min-w-[210px] p-2 bg-white drop-shadow-xl border mt-2 rounded-lg z-50"
                                v-if="isOtherUserDropdown" @click.stop>
                                <li>
                                    <button type="button"
                                            class="cursor-pointer decoration-0 block w-full px-5 py-3 font-medium text-start bg-transparent duration-500 hover:bg-gray-300 rounded-lg"
                                            @click="chatClear()" v-if="!chatClearLoading">
                                        Clear Chat
                                    </button>
                                    <button type="button"
                                            class="cursor-pointer decoration-0 flex justify-center items-center w-full px-5 min-h-[48px] max-h-[48px] font-medium text-start bg-transparent duration-500 hover:bg-gray-300 rounded-lg"
                                            v-if="chatClearLoading">
                                        <span
                                            class="inline-block size-[20px] border-2 border-transparent border-t-2 border-t-black rounded-full animate-spin"></span>
                                    </button>
                                </li>
                                <li>
                                    <button type="button"
                                            class="cursor-pointer decoration-0 block w-full px-5 py-3 font-medium text-start bg-transparent duration-500 hover:bg-gray-300 rounded-lg"
                                            @click="closeOtherUserDropdown();selectedUserInitials = ''">
                                        Exit Chat
                                    </button>
                                </li>
                            </ul>
                    ...
                    <div class="w-full h-[calc(100vh-325px)] bg-gray-200 p-3 scrollbar overflow-y-auto">
                        <div v-for="(message) in messages" :key="message.id" class="flex mb-3"
                            ...
                            <template v-if="message.sender_id != profileData.id">
                                <div
                                    class="min-w-[50px] min-h-[50px] inline-flex bg-white justify-center font-medium items-center rounded-full me-3 shadow-lg">
                            ...
                            <template v-else>
                                ...
                                <div
                                    class="min-w-[50px] min-h-[50px] inline-flex justify-center bg-blue-600 text-white font-medium items-center rounded-full ms-3 shadow-lg">
                    ...
                    <form @submit.prevent="manageChat()" class="p-3 w-full flex justify-between items-center">
                        <input type="text" name="message" v-model="formData.content" placeholder="Typing Here ..."
                               ...
                               autocomplete="off"/>
                        <button type="submit"
                                class="size-[50px] min-w-[50px] min-h-[50px] w-[50px] h-[50px] outline-0 border-0 bg-gray-200 inline-flex justify-center items-center group rounded-full ms-3">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                 stroke="currentColor" class="size-6 duration-500 rotate-0 group-hover:-rotate-45">
                                <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <!-- edit profile modal -->
    <div class="fixed inset-0 p-5 size-full flex justify-center items-center duration-500 z-50"
         :class="{ 'invisible bg-black/35' : !isEditProfileModal, 'visible bg-black/65' : isEditProfileModal }"
         @click="isEditProfileModal = false">
        <form @submit.prevent="editProfile()"
              class="bg-white rounded-2xl w-full max-w-[550px] duration-500 origin-top p-10"
              :class="{ 'translate-y-0 opacity-100' : isEditProfileModal, '-translate-y-1/2 opacity-0' : !isEditProfileModal }"
              @click.stop>
            ...
            <div class="flex justify-between items-center mb-3">
                ...
                <!-- modal close btn -->
                <button type="button"
                        class="size-[45px] inline-flex justify-center items-center bg-transparent duration-500 hover:bg-gray-300 rounded-full"
                        @click="closeEditProfileModal()">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                         class="size-6 stroke-black pointer-events-none">
            ...
            <div class="w-full block">
                ...
                <div class="block w-full mb-3">
                    ...
                    <input id="name" type="text" name="name" v-model="profileParam.name"
                           class="w-full block px-4 py-3 rounded-xl outline-0 border border-gray-300 focus-within:border-blue-600 duration-500"
                           autocomplete="off"/>
                ...
                <div class="block w-full mb-3">
                    ...
                    <input id="phone" type="text" name="phone" v-model="profileParam.phone"
                           class="w-full block px-4 py-3 rounded-xl outline-0 border border-gray-300 focus-within:border-blue-600 duration-500"
                           autocomplete="off"/>
            ...
            <div class="flex justify-end items-center gap-3">
                ...
                <button type="button"
                        class="outline-0 border-0 bg-gray-200 duration-500 hover:bg-gray-700 text-black hover:text-white inline-flex justify-center items-center min-w-[120px] min-h-[48px] max-h-[48px] rounded-xl"
                        @click="closeEditProfileModal()">
                    Cancel
                </button>

                <button type="submit"
                        class="outline-0 border-0 bg-blue-200 duration-500 hover:bg-blue-600 text-black hover:text-white inline-flex justify-center items-center min-w-[120px] min-h-[48px] max-h-[48px] rounded-xl"
                        v-if="!editProfileLoading">
                    Update
                </button>
                <button type="button"
                        class="outline-0 border-0 bg-blue-200 duration-500 hover:bg-blue-600 text-black hover:text-white inline-flex justify-center items-center min-w-[120px] min-h-[48px] max-h-[48px] rounded-xl group"
                        v-if="editProfileLoading">
                    <span
                        class="inline-block size-[20px] rounded-full border-2 border-transparent border-t-2 border-t-blue-600 group-hover:border-t-white animate-spin"></span>
                </button>
    ...
    <div class="fixed inset-0 p-5 size-full flex justify-center items-center duration-500 z-50"
         :class="{ 'invisible bg-black/35' : !isChangePasswordModal, 'visible bg-black/65' : isChangePasswordModal }"
         @click="isChangePasswordModal = false">
        <form @submit.prevent="changePassword()"
              class="bg-white rounded-2xl w-full max-w-[550px] duration-500 origin-top p-10"
              :class="{ 'translate-y-0 opacity-100' : isChangePasswordModal, '-translate-y-1/2 opacity-0' : !isChangePasswordModal }"
              @click.stop>
            ...
            <div class="w-full block">
                ...
                <div class="block w-full mb-3">
                    ...
                    <input id="current_password" type="password" name="current_password"
                           placeholder="Enter your current password" v-model="passwordParam.current_password"
                           class="w-full block px-4 py-3 rounded-xl outline-0 border border-gray-300 focus-within:border-blue-600 duration-500"
                           autocomplete="off"/>
                ...
                <div class="block w-full mb-3">
                    ...
                    <input id="new_password" type="password" name="new_password" placeholder="Enter your new password"
                ...
                <div class="block w-full mb-3">
                    <label for="new_password_confirmation" class="block mb-1 w-full font-medium"> New Confirm
                        Password </label>
    ...
    <!-- chat delete modal -->
    <div class="fixed inset-0 p-5 size-full flex justify-center items-center duration-500 z-50"
         :class="{ 'invisible bg-black/35' : !isChatDeleteModal, 'visible bg-black/65' : isChatDeleteModal }"
         @click="isChatDeleteModal = false">
        <form @submit.prevent="chatDelete()"
              class="bg-white rounded-2xl w-full max-w-[500px] duration-500 origin-top px-10 py-16"
              :class="{ 'translate-y-0 opacity-100' : isChatDeleteModal, '-translate-y-1/2 opacity-0' : !isChatDeleteModal }"
              @click.stop>
            ...
            <div class="mb-3 flex justify-center">
                <svg viewBox="0 0 24 24" class="size-[85px]" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <g id="SVGRepo_iconCarrier">
                        <path class="stroke-rose-600" d="M20.5001 6H3.5" stroke-width="1.5"
                              stroke-linecap="round"></path>
                        <path class="stroke-rose-600"
            ...
            <div class="flex justify-center items-center gap-3">
                ...
                <button type="button"
                        class="outline-0 border-0 bg-gray-200 duration-500 hover:bg-gray-700 text-black hover:text-white inline-flex justify-center items-center min-w-[120px] min-h-[48px] max-h-[48px] rounded-xl"
                        @click="closeChatDeleteModal()">
                    Cancel
                </button>

                <button type="submit"
                        class="outline-0 border-0 bg-rose-200 duration-500 hover:bg-rose-600 text-rose-900 hover:text-white inline-flex justify-center items-center min-w-[120px] min-h-[48px] max-h-[48px] rounded-xl"
                        v-if="!deleteLoading">
                    Confirm
                </button>

                <button type="button"
                        class="outline-0 border-0 bg-rose-200 duration-500 hover:bg-rose-600 text-rose-900 hover:text-white inline-flex justify-center items-center min-w-[120px] min-h-[48px] max-h-[48px] rounded-xl group"
                        v-if="deleteLoading">
                    <span
                        class="inline-block size-[20px] rounded-full border-2 border-transparent border-t-2 border-t-rose-600 group-hover:border-t-white animate-spin"></span>
                </button>
